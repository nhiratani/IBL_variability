...
def calc_trial_vecs(session_id, spikes, region_cluster_ids, hy_params):
    ...
    for tridx in range(Ntrials):
        ...
        # inclusion criteria
        # the task execution time satisfies: min_FT < FT < max_FT
        # high contrast trial
        # correct trial
        if (hy_params['min_FT'] <= FT and FT < hy_params['max_FT'])\
            and ( (trials_data['contrastLeft'][tridx] in high_cs) or (trials_data['contrastRight'][tridx] in high_cs) )\
            and trials_data['feedbackType'][tridx] > 0.5:
