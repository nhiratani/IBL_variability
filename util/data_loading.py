...
def get_behavioral_stats(session_id, hy_params):
    ...
    data = {}
    fname = "data/bdata_ephys/calc_behav_stats_eid" + str(session_id) + ".txt"
...
def extract_hash_eids(folder_name="data/bdata_ephys"):
    import re
    from pathlib import Path
    pattern = re.compile(r"^calc_behav_stats_eid(.+)\.txt$")
    eids = set()
    dir_path = Path(folder_name)
    
    # Ensure the directory exists before trying to scan
...
def load_data(session_type, one):
    ...
    for idx, eid in enumerate(eids):
        if session_type == 'ephys': 
            fname = "data/bdata_ephys/calc_behav_stats_eid" + str(eid) + ".txt"
            #fname = "util/wdata/calc_behav_stats_eid" + str(eid) + ".txt"
        elif session_type == 'all_biased':
            fname = "data/bdata_all/calc_trial_stats_eid" + str(eid) + ".txt"
...
def read_region_data(params):
    ...
    #if params['cluster_qc'] == 0.0: # legacy code
    #    session_ids, sess_infos = one.search(task_protocol= '_iblrig_tasks_ephysChoiceWorld6.*', details=True)
    
    if params['cluster_qc'] >= 0.5:
        session_ids = []
        fn_parent = "rdata/list_of_sesssion_for_" + str(params['region_group']) + "_regions_qc" + str(0.0) + "_minN" + str(10) + ".txt"
        for line in open(fn_parent, 'r'):
            ltmps = line.split(' ')
            session_id = str(ltmps[1])
