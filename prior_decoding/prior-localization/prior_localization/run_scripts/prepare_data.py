...
def prepare_ephys(
    ...
    for region in regions:
        ...
        if sum(region_mask) < min_units:
            ...
            # ------------------------------------------------------------
            # FIX: idxs_used are cluster IDs (values in spikes['clusters']),
            #      so DON'T use .iloc. Use .loc.
            # ------------------------------------------------------------
    ...
    # NOTE: signature changed: now also returns cluster_ids_list
...
def prepare_behavior(
    ...
    - Runs behavior target computation in the ORIGINAL trial space (trials_df length, e.g. 425)
    - Then, if keep_idx is provided, slices trials/targets/masks down to the subgroup space
      so returned all_trials/all_targets/all_masks match ephys subgroup length (e.g. 47).
    """
    ...
    n_trials_full = len(trials_df)

    if len(trials_mask) != n_trials_full:
        raise ValueError(
            f"prepare_behavior: trials_mask len {len(trials_mask)} != trials_df len {n_trials_full}"
        )

    if keep_idx is not None:
        keep_idx = np.asarray(keep_idx).astype(int)
        if keep_idx.ndim != 1:
            raise ValueError("prepare_behavior: keep_idx must be 1D")
        if keep_idx.min() < 0 or keep_idx.max() >= n_trials_full:
            raise ValueError("prepare_behavior: keep_idx out of bounds for trials_df")
    ...
    if -1 in pseudo_ids:
        ...
        if actual_target.shape[0] != n_trials_full:
            raise ValueError(
                f"prepare_behavior: actual_target len {actual_target.shape[0]} != trials_df len {n_trials_full}. "
                f"(compute_beh_target is not returning full trial-space target)"
            )
        actual_target_mask = np.isfinite(actual_target)

    # Collect targets for pseudo + actual
    all_targets = []
    all_trials = []
    all_masks = []
    all_neurometrics = []
    ...
    for n in range(n_pseudo_sets):
        ...
        for pseudo_id in pseudo_ids:
            if pseudo_id == -1:
                ...
                control_targets = np.asarray(control_targets)
                if control_targets.shape[0] != n_trials_full:
                    raise ValueError(
                        f"prepare_behavior: control_targets len {control_targets.shape[0]} != trials_df len {n_trials_full} "
                        f"for pseudo_id={pseudo_id}. (compute_beh_target not in full trial space)"
                    )

                target_mask = np.isfinite(control_targets)
                ...
                if config['estimator'] == sklm.LogisticRegression:
                    ...
                    while not logisticreg_criteria(targets_masked):
                        assert sample_pseudo_count < 100
                        sample_pseudo_count += 1
                        control_trials = generate_null_distribution_session(
                            trials_df, session_id, subject, model, behavior_path
                        )
                        control_targets = np.asarray(compute_beh_target(
                            control_trials, session_id, subject, model, target, behavior_path
                        ))
                        if control_targets.shape[0] != n_trials_full:
                            raise ValueError(
                                f"prepare_behavior: resampled control_targets len {control_targets.shape[0]} "
                                f"!= trials_df len {n_trials_full}"
                            )
                        target_mask = np.isfinite(control_targets)
                        control_mask = trials_mask & target_mask
                        targets_masked = control_targets[control_mask]
            ...
            if config['estimator'] == sklm.LogisticRegression:
                ...
                if not logisticreg_criteria(targets_masked):
                    logger.warning(
                        f'Target failed logistic regression criteria for session {session_id}, '
        ...
        if compute_neurometrics:
            ...
            for i in range(len(pseudo_ids)):
                ...
                neurometrics = neurometrics[set_masks[i]].reset_index(drop=True)
                set_neurometrics.append(neurometrics)
        else:
    ...
    # NEW: slice outputs to subgroup

    if keep_idx is not None:
        # keep only subgroup trials in every pseudo + actual array/mask
        for set_i in range(len(all_trials)):
            for j in range(len(all_trials[set_i])):
                all_trials[set_i][j] = all_trials[set_i][j].iloc[keep_idx].reset_index(drop=True)
                all_targets[set_i][j] = np.asarray(all_targets[set_i][j])[keep_idx]
                all_masks[set_i][j] = np.asarray(all_masks[set_i][j])[keep_idx]
