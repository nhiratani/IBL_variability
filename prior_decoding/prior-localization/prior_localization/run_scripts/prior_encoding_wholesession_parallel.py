...
OUT_ROOT = Path("./prior_localization_sessionfit_output_whole_session_behav_sessions")
OUT_ROOT.mkdir(parents=True, exist_ok=True)

# Tag used in output filenames
TAG = "WHOLE_SESSION_BEHAV_SESSIONS_UPDATED_ROIS"
...
# Still drop last N trials for decoding (set to 0 to disable)
DROP_LAST_N = int(os.getenv("DROP_LAST_N", "40"))
MIN_RAW_TRIALS = int(os.getenv("MIN_RAW_TRIALS", "401"))

# Whole-session metrics require enough decoded trials
...
def make_one_for_worker() -> ONE:
    ...
    # Helpful: show whether the expected host folder exists
...
def run_one_eid_worker(eid: str, out_root_str: str, n_pseudo: int, debug: bool):
    ...
    try:
        ...
        print(f"[LOAD] eid={eid} trials={n_raw_trials} (cache_dir={one.cache_dir})")
    except Exception as e:
        msg = f"skip_load_trials:{type(e).__name__}:{e}"
        print("[SKIP]", eid, msg)
        return {"eid": eid, "status": msg, "n_raw_trials": None, "rows": []}
    ...
    (out_root / eid / f"whole_session_summary_{TAG}_{roi_tag}.json").write_text(
...
def main():
    ...
    # Load unique session IDs (one eid per line)
    ...
    roi_tag = "_".join(ROI_LIST) if ROI_LIST else "ALL"
    summary_path = OUT_ROOT / f"whole_session_summary_{TAG}_{roi_tag}.pkl"
    with open(summary_path, "wb") as f:
        pickle.dump(all_rows, f)

    run_log_path = OUT_ROOT / f"run_log_whole_session_{TAG}_{roi_tag}.json"
    with open(run_log_path, "w") as f:
        json.dump(run_log, f, indent=2)

    print("Saved whole-session summary:", summary_path)
