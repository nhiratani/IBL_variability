...
def calc_acf_unbiased(trial_bins, spike_counts, hy_params):
    ...
    for tridx in range(Ntrial): 
        if tridx < bwin//2:
            ...
            tr_start = int(tridx - bwin//2); tr_end = int(tr_start + bwin-1)
        ...
        if bwin == 1: # use the trial-wise mean and variance
            ...
            spike_counts_bwin = np.concatenate( spike_counts[tr_start:tr_end+1] )  
...
def compute_ITI_acf(one, session_id, pid, region_of_interest, hy_params, atlas):
    try:       
        ...
        elif hy_params['ITI_def'] == 'init_period': # only use initial [ITI_max_dur - prestim_period] period of ITI
            ITI_starts = trial_end_times[:-1]
        ...
        for cluster_id, cluster_label, acronym in zip(clusters['cluster_id'], clusters['label'], clusters['acronym']):                                                              
            # good cluster criterion, matching the session selection in data_loading.process_single_pid
            if cluster_label >= hy_params['cluster_qc'] and region_of_interest == classify_acronym(acronym, hy_params['region_group']):  
        ...
        for tridx, (ITI_start, ITI_end) in enumerate( zip(ITI_starts, ITI_ends) ):
            ...
            if len(bins) >= acf_bin_max: # only use the ITI longer than acf_bin_max (exclude last forty too?)
                trial_bins.append( len(spike_counts) )
...
def compute_ITI_neuronwise_acf(one, session_id, pid, region_of_interest, hy_params, atlas):
    try:       
        insertions = one.alyx.rest('insertions', 'list', session=session_id)
        sl = SpikeSortingLoader(pid=pid, one=one, atlas=atlas)
        spikes, clusters, channels = sl.load_spike_sorting()
        ...
        for cluster_id in region_cluster_ids:
            ...
            total_spike_count = 0; total_dur = 0.0
            
            cluster_spikes_idx = np.where( spikes['clusters'] == cluster_id )[0]
            for tridx, (ITI_start, ITI_end) in enumerate( zip(ITI_starts, ITI_ends) ):
                SP_idx = np.where( (spikes['times'][cluster_spikes_idx] >= ITI_start) & (spikes['times'][cluster_spikes_idx] <= ITI_end) )[0]
                
                cluster_spike_times = spikes['times'][cluster_spikes_idx][SP_idx] 
                total_spike_count += len(cluster_spike_times)
                total_dur += ITI_end - ITI_start 
                ...
                if len(bins) >= acf_bin_max: # only use the ITI longer than acf_bin_max (exclude last forty too?)
                    trial_bins.append( len(spike_counts) )
                    cluster_spike_counts.append( spike_counts ) 
            
            cluster_firing_rate = total_spike_count / (total_dur + 1e-6)    
            if cluster_firing_rate >= hy_params['min_firing_rate']:
                bin_time, acf_value = calc_acf_unbiased(np.array(trial_bins), cluster_spike_counts, hy_params)
                if (not np.any( np.isnan(acf_value) ) ):
                    bin_times.append( bin_time ) 
                    acf_values.append( acf_value )
        
        print( len(ITI_starts), len(acf_values) )
