        ...
        for spid in spids:
            ...
            if results != None:
                ...
                if hy_params['tau_num'] == 'double':
                    ...
                    raise ValueError("Unknown fitting model specified")
        ...
        for spid in spids:
            ...
            if results != None:
                ...
                for nidx, acf_value in enumerate(acf_values):
                    ...
                    if hy_params['tau_num'] == 'double':
                        fitted_params, fitting_error = fit_acf_model_trm_double(bin_times, acf_value, hy_params, if_print=False)
                    else:
                        raise ValueError("Unknown fitting model specified")
                    fwtmp = str(session_id) + ' ' + str(pid) + ' ' + str(fitting_error)
                    for fitted_param in fitted_params:
...
def ITI_neuronwise_acf_fitting(region_of_interests, hy_params):
    for region_of_interest in region_of_interests:
        ...
        for spid in spids:
            ...
            results = compute_ITI_neuronwise_acf(one, session_id, pid, region_of_interest, hy_params, atlas=AllenAtlas())
            if results != None:
                bin_times, acf_values = results
                for nidx, (bin_time, acf_value) in enumerate( zip(bin_times, acf_values) ):
                    fwtmp = str(session_id) + ' ' + str(pid) + ' ' + str(nidx) + ' '
                    for val in acf_value:
                        fwtmp += str(val) + ' '
                    fw.write(fwtmp + '\n')
                    ...
                    if hy_params['tau_num'] == 'double':
                        fitted_params, fitting_error = fit_acf_model_trm_double(bin_time, acf_value, hy_params)
                    else:
                        raise ValueError("Unknown fitting model specified")
...
if __name__ == "__main__":
    ...
    SA_hy_params = {
        ...
        'n_iter': 1000000, #0, #100000, # number of iterations for trust-region-method fitting (n_iter =  1000000; except neuronwise fitting where n_iter =  100000)
        'n_seeds': 10000, #0, (n_seeds = 10000, except neuronwise fitting where n_seeds = 1000)
    ...
    ITI_full_hy_params = {
        ...
        'ITI_max_dur': 2.0, # [s] ITI_max duration time for conditional/init_period ITI analysis
        'pre_stim_period': 0.5, # [s] stimOn_time - pre_stim_period is defined as the end of ITI
        'bin_window': 30, #(10, 30, 100), # number of trials for estimating the mean and variance of auto-correlation
