...
def plot_tau_trait_comparisons(region_stats, session_behav_stats, sbj_behav_stats, hy_params, behav_params, params_str, data_type):
    ...
    MedVisColors = (mpl.colormaps['tab10'].colors)[6:7] + (mpl.colormaps['tab10'].colors)[8:]
    ...
    if data_type == 'SA' or data_type == 'ITI':
        ...
        for region in region_stats.keys():
            if (data_type == 'SA' and (region in region_division['Medial-VIS'])) or (data_type == 'ITI' and region == 'VISa'):              
                ...
                #plt.scatter( xregs, yregs, color=region_colors[rcidx], label=region)
                plt.scatter( xregs, yregs, color=MedVisColors[rcidx], label=region)

                #plt.scatter( xregs, yregs, color='purple', label=region, alpha=0.5)
                #plt.axvline( np.mean(xregs), color=region_colors[rcidx] )
                plt.axvline( np.mean(xregs), color=MedVisColors[rcidx] )
                xdivs.extend(xregs); ydivs.extend(yregs)
        ...
        #slope, intercept, r, p, se = scist.linregress( xdivs, ydivs )
        #plt.plot(xs, slope*xs + intercept, color='k', lw=1.0)
        beta1, beta0 = deming_regression(xdivs, ydivs)
        plt.plot(xs, beta1*xs+beta0, color='k', lw=1.0)
    ...
    scnt_region_names = []; region_session_count = []; region_tot_neuron_count = []; scnt_region_colors = []
    for didx, division in enumerate( div_corr.keys() ):
        for region in region_corr.keys():
            if region in region_division[division]:     
                scnt_region_names.append(region)
                region_session_count.append( len(region_stats[region]) )
                scnt_region_colors.append( region_color_palatte[2*didx+1] ) 
    
    fig6b = plt.figure(figsize=(5.4, 10.8))
    plt.barh(scnt_region_names[::-1], region_session_count[::-1], color=scnt_region_colors[::-1], alpha=0.6)
    plt.subplots_adjust(left=0.3, right=0.95)
    plt.yticks(rotation=180)
    plt.show()
    fig6b.savefig( "figs/fig_neural/" + data_type + "_valid_session_count_list" + params_str + ".pdf" ) 
    ...
    plt.show()
    print( 'F count: ', len(sbj_xs['F']), 'M count: ', len(sbj_xs['M']) )
    ...
    # F/M t-test with subsampling
    """
    N_female = len(sbj_xs['F']); N_male = len(sbj_xs['M'])
    print(N_female, N_male)
    female_impul = np.array(sbj_xs['F']); male_impul = np.array(sbj_xs['M']) 
    ks_res = {'stat':[], 'pval':[]}
    Welch_res = {'stat':[], 'pval':[]}
    for i in range(1000):
    ...
    print( 'F/M KS-test (subsampling): stat:', np.median(ks_res['stat']), ', pval: ', np.median(ks_res['pval']) )
    print( 'F/M Welch t-test (subsampling): stat:', np.median(Welch_res['stat']), ', pval: ', np.median(Welch_res['pval']) )
    """
    ...
    print( 'a2v-corr N: ', len(sbj_xs['tot']) )
    ...
    for feature in behav_features:
        ...
        if feature == 'ratio_fast':
            lr_params = scist.linregress(sbj_xs, sbj_ys)
    ...
    sbj_ys = {'F':[], 'M':[], 'tot':[]};
    for subject in sbj_behav_stats.keys():
        if subject in sbj_taus.keys():
            sbj_sex = sbj_behav_stats[subject]['sex']
            sbj_xs[sbj_sex].append( sbj_taus[subject] )
            sbj_ys[sbj_sex].append( sbj_behav_stats[subject]['ratio_fast'] )
    sbj_xs['tot'] = sbj_xs['F'] + sbj_xs['M']
    sbj_ys['tot'] = sbj_ys['F'] + sbj_ys['M']
    #rho_hat, rho_intercept, pvalue = calc_perm_significance(sbj_xs['tot'], sbj_ys['tot'])
    #print('sbj ATI vs. ratio_fast : ', rho_hat, rho_intercept, pvalue)
    for sex in ['F', 'M']:
        plt.scatter( sbj_xs[sex], sbj_ys[sex] )
    xs = np.arange( np.min(sbj_xs['tot']), np.max(sbj_xs['tot']), 0.01 )
    plt.plot(xs, lr_params.slope * xs + lr_params.intercept, color='k')
    plt.ylim(-0.01, 0.35)
    
    plt.show() 
    fig9.savefig( "figs/fig_neural/" + data_type + "_behav_timescale_ratio_fast_comparison_animal_wise_list_" + params_str + ".pdf" )
...
def plot_hierarchy_scores(region_stats, hy_params, behav_params, params_str, data_type):
    ...
    xtmps = np.arange(np.min(xs), np.max(xs), 0.01)
    ytmps = rho_intercept + rho_hat * xtmps
    plt.plot( xtmps, ytmps, color='k', lw=1.0 )
...
def SA_behav_timescale_comparison(SA_hy_params, behav_params):
    ...
    params_str = '_taunum_' + str(SA_hy_params['tau_num']) + '_model_' + str(SA_hy_params['model']) + '_proj_' + str(SA_hy_params['projection']) \
                + '_qc' + str(SA_hy_params['cluster_qc']) + '_minN' + str(SA_hy_params['min_neurons']) + '_min_sp' + str(SA_hy_params['min_total_spikes'])\
                + '_bs' + str(SA_hy_params['bin_size']) + '_acfbm' + str(SA_hy_params['acf_bin_max'])\
                + '_niter' + str(SA_hy_params['n_iter']) + '_sctf' + str(behav_params['s_cutoff']) + '_nseeds' + str(SA_hy_params['n_seeds'])
...
def SA_neuronwise_timescale_analysis(SA_hy_params, behav_params):
    params_str = 'neuronwise_taunum_' + str(SA_hy_params['tau_num']) + '_model_' + str(SA_hy_params['model']) + '_qc' + str(SA_hy_params['cluster_qc'])\
...
def ITI_neuronwise_timescale_analysis(ITI_hy_params, behav_params):
    params_str = 'neuronwise_taunum_' + str(ITI_hy_params['tau_num']) + '_model_' + str(ITI_hy_params['model']) + '_proj_' + str(ITI_hy_params['projection']) + '_ITI_def_' + str(ITI_hy_params['ITI_def'])\
            + '_wbin' + str(ITI_hy_params['bin_window']) + '_itimaxd' + str(ITI_hy_params['ITI_max_dur']) + '_ptstp' + str(ITI_hy_params['pre_stim_period'])\
            + '_qc' + str(ITI_hy_params['cluster_qc']) + '_minN' + str(ITI_hy_params['min_neurons']) + '_min_sp' + str(ITI_hy_params['min_total_spikes'])\
    ...
    region_stats, session_behav_stats, sbj_behav_stats = load_neuronwise_ac_data(ITI_hy_params, behav_params, params_str, 'ITI')
    plot_tau_trait_comparisons(region_stats, session_behav_stats, sbj_behav_stats, ITI_hy_params, behav_params, params_str, 'ITI')
    #plot_tau_tau_comparison(region_stats, sbj_behav_stats, SA_hy_params, behav_params, params_str, 'SA')
    plot_hierarchy_scores(region_stats, ITI_hy_params, behav_params, params_str, 'ITI')
...
def autocorr_value_plot(ITI_hy_params):
    ...
    bin_windows = [1, 30, 3000]#[1, 30, 3000]
    fig1 = plt.figure(figsize=(5.4, 4.8))
...
if __name__ == "__main__":
    ...
    SA_hy_params = {
        ...
        'n_iter': 1000000, #0, # (1000000 for population fitting) # number of maximum iterations for curve_fit
        'n_seeds': 10000, #0, # (10000 for population fitting) number of seeds for curv_fit
        #'tau_threshold': 0.03, # max tau1 and min tau2
        'tau_num': 'double', #'double', # number of exponential decays
        ...
        'max_error': 0.025, #maximum fitting error (0.025 for population fitting; 0.01 for neuron-wise fitting)
    ...
    ITI_full_hy_params = {
        ...
        'ITI_max_dur': 2.0,  #2.0# maximum ITI duration for fixed ITI analysis
        ...
        'trait': 'impulsivity',
        'max_error' : 0.025, #0.025, #0.025, # maximum fitting error
